...
@section('title', (isset($sharedRole) ? 'Editar' : 'Adicionar') . ' Papel de Partilha')

@section('breadcrumb')
    <li class="breadcrumb-item"><a class="text-dark" href="{{ route('admin.shared-roles.index') }}">Papéis de partilha</a>
    </li>
...
@section('content')
    <section class="content">
        <form
            action="{{ isset($sharedRole) ? route('admin.shared-roles.update', $sharedRole->id) : route('admin.shared-roles.store') }}"
            method="post">
            @csrf
            @if (isset($sharedRole))
                @method('PUT')
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="code"
                                    value="{{ isset($sharedRole) ? $sharedRole->code : '' }}" class="validate form-control"
                                    required>
                        ...
                        <div class="card-header p-2">
                            <ul class="nav nav-pills">
                                @foreach ($languages as $key => $language)
                                    <li class="nav-item">
                                        <a @class(['nav-link', 'active' => $key == 0]) href="#{{ $language->code }}"
                            ...
                            <div class="tab-content">
                                @foreach ($languages as $key => $language)
                                    <div @class(['tab-pane', 'active' => $key == 0]) id="{{ $language->code }}">
                                        <div class="form-group">
                                            <label for="inputDisplayName">Nome em {{ $language->name->pt }}
                                                <span class="text-danger">*</span>
                                            </label>
                                            <input type="text"
                                                value="{{ isset($sharedRole) ? $sharedRole->name->{$language->code} ?? '' : '' }}"
                                                name="name[{{ $language->code }}]" class="validate form-control" required>
            ...
            <div class=" row">
                <div class="col-12">
                    ...
                    <button type="submit"
                        class="btn btn-success float-right">{{ isset($sharedRole) ? 'Editar' : 'Adicionar' }}</button>
                </div>
            </div>
        </form>
    </section>
